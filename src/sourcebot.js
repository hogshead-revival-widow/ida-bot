...
class SourceBot {
  ...
  async run () {
    ...
    this.tabRunner = new TabRunner(tab.id)
  ...
  handleAction (action) {
    ...
    if (action.fill && 'isQuery' in action.fill) {
      action.fill.value = this.articleInfo.query
    }
    if (action.fill && 'isSource' in action.fill) {
      const params = this.getParams()
      action.fill.value = params.dbShortcut
    }
    return action
  }
